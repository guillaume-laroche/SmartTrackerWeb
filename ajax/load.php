			...
			foreach($dataArray as $key => $tracking) {
				try {
					...
					if($tracking["type"] == "event") {
						...
						$sql = "INSERT INTO TRACKING_EVENTS(TRE_TRK_ID, TRE_LINE, TRE_TIMESTAMP, TRE_EVENT_TYPE, TRE_LATITUDE, TRE_LONGITUDE, TRE_ALTITUDE, TRE_SPEED, TRE_DISTANCE, TRE_ACCURACY, TRE_RELEVANT)
							  	VALUES(" . $tracking_id . ", " . $line . ", str_to_date('" . $tracking["timestamp"] . "', '%d-%m-%Y %H:%i:%s'), '" . $tracking["type"] . "', " . $tracking["latitude"] . ", " . $tracking["longitude"] . ", " . $altitude . ", " . $speed . ", " . $distance . ", " . $accuracy . ", " . $tracking["relevant"] . ")";
						$conn->exec($sql);	  		
					}
					$line++;				
				}			
